...
export const REGEX_LIBRARY: RegexTemplate[] = [
  ...
  { id: 'any-number', name: '任意数字', pattern: '-?\\d+(\\.\\d+)?', flags: '', desc: '整数或小数,可带负号', example: '-12.5' },
  { id: 'two-decimal', name: '两位小数', pattern: '-?\\d+\\.\\d{2}', flags: '', desc: '恰好两位小数,如 12.34', example: '12.34' },
  { id: 'percent', name: '百分数', pattern: '-?\\d+(\\.\\d+)?%', flags: '', desc: '百分比,如 25% / 12.5%', example: '12.5%' },
  { id: 'scientific', name: '科学计数法', pattern: '-?\\d+(\\.\\d+)?[eE][+-]?\\d+', flags: '', desc: '如 1.5e-3', example: '1.5e-3' },
  { id: 'thousands', name: '千分位金额', pattern: '\\d{1,3}(,\\d{3})+(\\.\\d{2})?', flags: '', desc: '千分位分隔,如 1,234.56', example: '1,234.56' },
