...
`diffText(textA, textB, mode)` MUST 计算两段文本差异并以 HTML 字符串返回高亮结果(新增绿底、删除红底、未变默认)。mode 支持 line/word/char。返回 `ToolResult<string>`。渲染 MUST 经 HTML 转义(无 XSS)。本能力对应「文本处理」工具的「对比」tab。
...
#### Scenario: 纯函数可测
- **WHEN** 直接调用 diffText
- **THEN** 返回确定的 `ToolResult<string>`,由 golden test 断言精确 HTML 输出
