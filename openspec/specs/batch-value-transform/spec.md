...
TBD — created by syncing `openspec/changes/batch-and-translate-tools`. Covers the offline batch value transform tool (`batch-transform`): mixed-format value parsing, an ordered composable operation pipeline, output format presets, and a pure-function core.
