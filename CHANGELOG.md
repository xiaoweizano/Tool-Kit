...
- 数据文本组:#14 文本处理(对比/统计/大小写/分词)、#17 进制转换(BigInt)、#21 日志分析(9 维分析+异常聚类+50MB 截断)
  - #14 扩展为 4-tab「文本处理」:原 对比(line/word/char 高亮+XSS 转义)新增 textStats 九维统计、applyCase 十种大小写模式、segmentText 按类别+自定义分隔符分词分段;对比 改为左右分栏(diffSideBySide)+合并(diffText)两种视图,可 左右对比/合并 切换
